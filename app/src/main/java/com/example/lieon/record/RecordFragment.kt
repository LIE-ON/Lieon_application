...
import com.example.lieon.db.RecordHistory
...
class RecordFragment : Fragment() {
    ...
    private fun saveRecordHistory() {
        lifecycleScope.launch {
            val start = System.currentTimeMillis()
            val end = System.currentTimeMillis() // End time should be properly captured when recording stops
            val recordHistory = RecordHistory(0, start.toString(), end.toString())
            database.recordHistoryDao().insert(recordHistory)
        }
    }
